...
export const GENERAL_ERR_MSG = "Something went wrong, try again!";
...
export function flashNow(flash) {
  ...
  const flashType = flash.type || "success";
  const classes = ["flash", "alert", flashType];
  const iconTypes = {
  ...
  `;
  const insertAfterEl = Array.from(document.querySelectorAll(".flash")).at(-1);
  if (insertAfterEl) {
