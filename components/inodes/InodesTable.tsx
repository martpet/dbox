...
import type { Context, User } from "../../util/types.ts";
...
export default function InodesTable(props: Props, ctx: Context) {
  const {
    inodes,
    canCreate,
  ...
  const isBox = path.segments.length === 1;
  ...
  return (
    <>
      <script type="module" src={asset("inodes/sort.js")} />
      ...
      >
        {inodes.length === 0 && (
          blankSlate || (
            <BlankSlate
              title={`Empty ${isBox ? "Box" : "Folder"}`}
              subTitle={canCreate
                ? `Upload a file or create a ${isBox ? "" : "sub"}folder.`
                : undefined}
            />
          )
        )}
