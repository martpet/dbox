...
import { QueueMsgChangeDirChildrenAcl } from "../kv/queue/change_dir_children_acl.ts";
...
import type { AclDiff, AclPreview, Inode } from "./types.ts";
...
export async function applyAclDiffs(opt: {
  ...
  for (const { userId, role } of diffs) {
    ...
    if (userId === actingUserId || isBox && userId === ownerId) {
      continue;
    }
