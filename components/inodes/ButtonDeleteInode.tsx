...
export default function ButtonDeleteInode(props: Props) {
  const { inode } = props;
  const inodeLabel = getInodeLabel(inode).toLowerCase();
  return (
    <button
      command="show-modal"
...
export function DialogDeleteInode({ inode }: Props) {
  const inodeLabel = getInodeLabel(inode).toLocaleLowerCase();
  ...
  return (
    <dialog id="delete-inode-dilaog">
      ...
      <form method="post" action="/inodes/delete" class="basic">
        ...
        <label>
          <span>
            Please type the name of the {inodeLabel.toLowerCase()} to continue:
          </span>
          <input
            type="text"
            // autofocus
