import { checkPubAccess } from "$util";
...
export default function ButtonEditAccess(props: Props) {
  ...
  const isPubAccess = checkPubAccess(inode);

  return (
    <button
      disabled
      class="inode-edit-acl nobtn wait-disabled"
      title={btnProps.title || "Change Access"}
      ...
      data-has-pub-access={isPubAccess ? "1" : null}
    >
      <i class={`ico-${isPubAccess ? "unlock" : "lock"}`} />
    </button>
  );
}
