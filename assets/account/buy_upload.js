...
checkoutSignal.subscribe((event) => {
  ...
  if (event.type === "idle") {
    ...
    loaderSignal.value = `Waiting for ${provider || "Provider"}`;
    errorSignal.value = null;
  } else if (event.type === "processing") {
    loaderSignal.value = "Confirming";
  } else if (event.type === "success") {
    flashNow(
      `${quantitySignal.value} GB of upload quota has been added to your account.`
    );
...
export function initDialog() {
  ...
  checkoutSignal.subscribe((event) => {
    if (event.type === "success") {
      renderUploadQuota(event.newUploadQuota);
    }
  });
}
...
function createCheckout(loadingMsg) {
  loaderSignal.value = loadingMsg;
...
function insertDialog() {
  document.body.insertAdjacentHTML(
    ...
    `
      <dialog id="buy-traffic-dialog">
        ...
        <div id="buy-traffic-summary" hidden>
          <p>
            Price:
            <strong>$<output id="buy-traffic-amount">1</output></strong>
          </p>
          <p>
            Quantity: 
            <input id="buy-traffic-quantity" type="number" min="1" max="100" value="1" />
            <span id="buy-traffic-gigabyte">gigabyte</span> 
          </p>
        </div>
        <div id="buy-traffic-loader" class="spinner-lg spinner-col"></div>
  ...
  productSummaryEl = document.getElementById("buy-traffic-summary");
...
function renderUploadQuota(quota) {
  if (uploadQuotaEl) uploadQuotaEl.textContent = quota;
}
