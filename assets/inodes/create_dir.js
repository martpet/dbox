...
const { parentDirId, isBox } = btnShowDialog.dataset;
const showPreview = isBox;
const inodeLabel = isBox ? "Box" : "Folder";
...
async function submitData() {
  ...
  if (resp.ok) {
    ...
    statusSignal.value = "closed";
    flashNow(`${inodeLabel} "${dirName}" created`);
  } else {
  ...
  document.body.insertAdjacentHTML(
    ...
    `
      <dialog id="create-dir-dialog">
        <h1>New ${inodeLabel.toLowerCase()}</h1>
