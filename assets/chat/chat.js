...
let olderMsgsCursor;
let lastSeenFeedItemId = rootEl.dataset.lastSeenFeedItemId;
let socket;
let newMsgSeen;
...
async function lazyLoadHistory() {
  ...
  if (mount) {
    ...
    if (resp.ok) {
      ...
      lastSeenFeedItemId =
        document.getElementById("chat-feed").dataset.lastSeenFeedItemId;
    } else {
...
function insertMessageDialogs() {
  ...
  rootEl.insertAdjacentHTML(
    ...
    `
      ...
      <dialog id="cleanup-inode-msg-dialog" class="chat-msg-dialog">
        <h1>Delete Chage Message</h1>
        <p class="alert warning">Are you sure you want to delete this message?</p>
        ${msgPreview()}
        <form method="dialog" class="basic">
          <footer>
            <button>Cancel</button>
            <button class="submit">Delete Now</button>
          </footer>
        </form>
      </dialog>
  `
