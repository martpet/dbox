...
export default async function createDirNodeHandler(ctx: Context) {
  ...
  if (isParentRoot && RESERVED_ROOT_DIR_NAMES.includes(dirName)) {
    const errMsg = `Sorry, box name '${dirName}' is reserved by the system.`;
  ...
  if (currentDir) {
    ...
    if (isParentRoot) {
      ...
      errMsg = `Box '${dirName}' is already created by ${who}`;
    } else {
