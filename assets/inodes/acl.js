...
const container = document.getElementById("inodes-wrapper");
...
async function submitChanges() {
  ...
  flashNow(`Updated access for ${inodeLabel.toLowerCase()} "${inodeName}"`);
  statusSignal.value = "closed";
}
  ...
  document.body.insertAdjacentHTML(
    ...
    `
        <dialog id="acl-dialog">
          <h1>${inodeLabel} Access</h1>
          <p class="inode-name">${inodeName}</p>
          <form id="acl-form" class="basic">
            <p id="acl-error" class="alert error" hidden></p>
            <p>
              <label>
                <input type="checkbox" id="pub-acl" ${pubAccessCheckAttr} />
            ...
            <fieldset class="permissions">
              ...
              <button id="acl-add-user" type="button"><i class="ico-plus-lg"></i>Add User</button>
              <p hidden id="acl-loading"><span class="spinner-sm"></span>Loading</p>
            </fieldset>
            <footer>
              <button type="button" id="acl-close">Cancel</button>
              <button id="acl-submit">Apply</button>
            </footer>
          </form>
        </dialog>
      `
...
function createAclEl(aclItem) {
  ...
  if (aclItem) {
    ...
    aclEl.insertAdjacentHTML("beforeend", createRoleSelectHtml());
  }
